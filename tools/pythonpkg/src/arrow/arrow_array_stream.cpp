...
PyArrowObjectType GetArrowType(const nb::handle &obj) {
	...
	if (nb::isinstance(obj, table_class)) {
		return PyArrowObjectType::Table;
	} else if (nb::isinstance(obj, record_batch_reader_class)) {
	...
	if (nb::isinstance(obj, scanner_class)) {
		return PyArrowObjectType::Scanner;
	} else if (nb::isinstance(obj, dataset_class)) {
...
nb::object PythonTableArrowArrayStreamFactory::ProduceScanner(nb::object &arrow_scanner, nb::handle &arrow_obj_handle,
	...
	if (has_filter) {
		...
		if (column_list.empty()) {
			return arrow_scanner(arrow_obj_handle, nb::arg("filter") = filter);
		} else {
			return arrow_scanner(arrow_obj_handle, nb::arg("columns") = projection_list, nb::arg("filter") = filter);
		}
	} else {
		if (column_list.empty()) {
			return arrow_scanner(arrow_obj_handle);
		} else {
			return arrow_scanner(arrow_obj_handle, nb::arg("columns") = projection_list);
		}
	}
}
unique_ptr<ArrowArrayStreamWrapper> PythonTableArrowArrayStreamFactory::Produce(uintptr_t factory_ptr,
	...
	case PyArrowObjectType::Table: {
		...
		nb::object arrow_scanner = dataset.attr("__class__").attr("scanner");
...
void PythonTableArrowArrayStreamFactory::GetSchemaInternal(nb::handle arrow_obj_handle, ArrowSchemaWrapper &schema) {
	auto table_class = nb::module_::import_("pyarrow").attr("Table");
	if (nb::isinstance(arrow_obj_handle, table_class)) {
...
nb::object GetScalar(Value &constant, const string &timezone_config, const ArrowType &type) {
	...
	case LogicalTypeId::DATE: {
		nb::object date_type = nb::module_::import_("pyarrow").attr("date32");
		return dataset_scalar(scalar(constant.GetValue<int32_t>(), date_type()));
	}
	case LogicalTypeId::TIME: {
		nb::object date_type = nb::module_::import_("pyarrow").attr("time64");
		return dataset_scalar(scalar(constant.GetValue<int64_t>(), date_type("us")));
	}
	case LogicalTypeId::TIMESTAMP: {
		nb::object date_type = nb::module_::import_("pyarrow").attr("timestamp");
		return dataset_scalar(scalar(constant.GetValue<int64_t>(), date_type("us")));
	}
	...
	case LogicalTypeId::UTINYINT: {
		nb::object integer_type = nb::module_::import_("pyarrow").attr("uint8");
		return dataset_scalar(scalar(constant.GetValue<uint8_t>(), integer_type()));
	}
	case LogicalTypeId::USMALLINT: {
		nb::object integer_type = nb::module_::import_("pyarrow").attr("uint16");
		return dataset_scalar(scalar(constant.GetValue<uint16_t>(), integer_type()));
	}
	case LogicalTypeId::UINTEGER: {
		nb::object integer_type = nb::module_::import_("pyarrow").attr("uint32");
		return dataset_scalar(scalar(constant.GetValue<uint32_t>(), integer_type()));
	}
	case LogicalTypeId::UBIGINT: {
		nb::object integer_type = nb::module_::import_("pyarrow").attr("uint64");
	...
	case LogicalTypeId::DECIMAL: {
		nb::object date_type = nb::module_::import_("pyarrow").attr("decimal128");
		uint8_t width;
		uint8_t scale;
		...
		default: {
			...
			auto hugeint_value_py = nb::cast(hugeint_value.upper);
...
nb::object TransformFilterRecursive(TableFilter *filter, vector<string> &column_ref, const string &timezone_config,
	...
	case TableFilterType::CONSTANT_COMPARISON: {
		...
		auto constant_field = field(nb::tuple(nb::cast(column_ref)));
	...
	case TableFilterType::CONJUNCTION_OR: {
		...
		nb::object expression = TransformFilterRecursive(child_filter, column_ref, timezone_config, type);
		while (i < or_filter.child_filters.size()) {
			child_filter = or_filter.child_filters[i++].get();
			nb::object child_expression = TransformFilterRecursive(child_filter, column_ref, timezone_config, type);
...
nb::object PythonTableArrowArrayStreamFactory::TransformFilter(TableFilterSet &filter_collection,
	...
	nb::object expression = TransformFilterRecursive(it->second.get(), column_ref, config.time_zone, arrow_type);
