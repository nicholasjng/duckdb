...
public:
	explicit PandasDataFrameBind(nb::handle &df) {
		names = nb::list(df.attr("columns"));
		types = nb::list(df.attr("dtypes"));
...
static LogicalType BindColumn(PandasBindColumn &column_p, PandasColumnBindData &bind_data,
	...
	if (bind_data.numpy_type.type == NumpyNullableType::CATEGORY) {
		...
		auto categories = nb::array(column.attr("cat").attr("categories"));
		...
		if (categories_pd_type.type == NumpyNullableType::OBJECT) {
			...
			vector<string> enum_entries = nb::cast<vector<string>>(categories);
			...
			auto pandas_col = nb::array(column.attr("cat").attr("codes"));
			...
			auto pandas_col = nb::array(column.attr("to_numpy")());
		...
		bind_data.pandas_col = make_uniq<PandasNumpyColumn>(nb::array(column.attr("to_numpy")("float32")));
		...
		if (nb::hasattr(pandas_array, "_data")) {
			...
			bind_data.pandas_col = make_uniq<PandasNumpyColumn>(nb::array(pandas_array.attr("asi8")));
		} else {
			// Otherwise we have to get it through 'to_numpy()'
			bind_data.pandas_col = make_uniq<PandasNumpyColumn>(nb::array(column.attr("to_numpy")()));
		}
		column_type = NumpyToLogicalType(bind_data.numpy_type);
	}
...
void Pandas::Bind(const ClientContext &context, nb::handle df_p, vector<PandasColumnBindData> &bind_columns,
	...
	for (idx_t col_idx = 0; col_idx < column_count; col_idx++) {
		...
		names.emplace_back(nb::str(df.names[col_idx]));
